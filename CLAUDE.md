...
**Open items / not yet done** (flagged in various sessions, still outstanding as of Session 19):
...
- No staging→production promotion path yet — `study-dev.tsys.com.ph` is a separate environment from the live `tsys.com.ph`/`www.tsys.com.ph` production site; nothing here currently pushes to production. **`robots.txt`, `sitemap.xml`, and `index.html`'s static OG/JSON-LD are hardcoded to `https://study-dev.tsys.com.ph`** (see Session 19) — these three files need a manual domain update if/when this build is ever promoted to replace the live `tsys.com.ph` site.
